...
В  SQLite такого типа нет, но используется модификатор типа AUTOINCREMENT. Используется с указанием первичного ключа
PRIMARY KEY.
В БД Postrgres тип SERIAL предназначен для данных размером 4 байт, целое число с автоувеличением от 1 до 2147483647.
В БД MySQL тип SERIAL - это псевдоним, который сочетает в себе тип столбца (BIGINT в частности),
AUTO_INCREMENT, UNSIGNED и другие атрибуты для определенного столбца.
